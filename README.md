...
flowchart LR
   subgraph SB["Secure Boot Variables (User-owned state)"]
      ...
      subgraph KEK["KEK Certificates"]
         ...
         MSKEK2011["Microsoft Corporation KEK CA 2011 ◆<br/>(signs Microsoft's dbx updates)"]:::expiring
         MSKEK2023["Microsoft Corporation KEK 2K CA 2023 ◆"]:::current
      end
      ...
      subgraph DB["DB Certificates"]
         ...
         MSWINPCA2011["Microsoft Windows Production PCA 2011 ◆<br/>(signs unmigrated Windows Boot Manager)"]:::expiring
         MSUEFICA2011["Microsoft Corporation UEFI CA 2011 ◆<br/>(signs shims and option ROMs in the field)"]:::expiring
         MSWINUEFICA2023["Windows UEFI CA 2023 ◆"]:::current
         MSUEFICA2023["Microsoft UEFI CA 2023 ◆"]:::current
         MSOPROM2023["Microsoft Option ROM UEFI CA 2023 ◆"]:::current
      end
   ...
   MSWINPCA2011 -.->|"signs unmigrated Windows Boot Manager"| BL_WIN
   MSWINUEFICA2023 -->|signs| BL_WIN
   MSUEFICA2011 -.->|"signs currently-shipped shims and option ROMs"| BL_SHIM
   MSUEFICA2023 -->|signs| BL_SHIM
   MSUEFICA2011 -.->|signs| OPROM
   MSOPROM2023 -->|signs| OPROM
...
SB-ENEMA enrolls **both** certificate generations — 2011 and 2023 — in db and KEK. It does not drop a certificate merely because it is expiring. See [Why the expiring 2011 certificates are still enrolled](#why-the-expiring-2011-certificates-are-still-enrolled) for the reasoning; the short version is that UEFI never checks certificate expiry dates, so removing an expiring CA breaks the binaries it signed without buying any security.
...
### Why the expiring 2011 certificates are still enrolled

Everything above is an argument for **adding** the 2023 certificates. It is not an argument for **removing** the 2011 ones, and SB-ENEMA does not remove them. Both generations are enrolled, in `db` and in `KEK`.

This is a deliberate reversal of earlier SB-ENEMA behaviour. Previous versions dropped `Microsoft Windows Production PCA 2011` from `db` and `Microsoft Corporation KEK CA 2011` from `KEK`, on the theory that both were superseded. Both exclusions were wrong:

- **`Microsoft Windows Production PCA 2011` signs the Windows Boot Manager on machines that have not yet migrated.** Microsoft's rollout of the 2023-signed boot manager is staged and, as of mid-2026, incomplete. It also covers WinRE, and any installation or recovery media created before the migration. A machine re-provisioned without this certificate has a `db` that cannot validate its own installed Windows — Secure Boot then refuses to boot the OS that was working ten minutes earlier. Note that shipping firmware carries this certificate in `dbDefault` for exactly this reason.

- **`Microsoft Corporation KEK CA 2011` signs Microsoft's revocation updates.** Every Microsoft Secure Boot servicing package published to date is signed under the 2011 KEK, not the 2023 one — verifiable against this repository: extract the PKCS#7 signer from `third_party/secureboot_objects/PostSignedObjects/DBX/*/DBXUpdate.bin` and every chain ends at `Microsoft Corporation KEK CA 2011`. A machine holding only the 2023 KEK cannot apply any Microsoft `dbx` update that currently exists — which is precisely the frozen-revocation-list failure described in point 2 above.

The reason removal buys nothing is the same fact the previous section opens with: **UEFI never checks certificate expiry.** The specification's authorization process has no expiry step, and EDK II's `Pkcs7Verify()` explicitly passes `X509_V_FLAG_NO_CHECK_TIME` so that a dead CMOS battery cannot brick a machine. So an expired-but-unrevoked CA in `db` behaves exactly as it did the day before it expired. Removing it does not revoke anything an attacker could use; it only breaks the legitimate binaries that CA signed.

Revocation in UEFI Secure Boot is expressed through `dbx`, and through `dbx` alone — and `dbx` wins over `db`. That is what makes carrying both generations safe: if Microsoft ever does revoke the 2011 Windows PCA in the default `dbx`, that revocation overrides the `db` entry automatically, with no change needed here. SB-ENEMA enforces this at build time — `scripts/check-secureboot-policy.py` fails the build if the generated `dbx` ever revokes a certificate the generated `db` depends on, so the two can never drift into a state that bricks the machines this tool is supposed to repair.

The governing principle is that SB-ENEMA is a **recovery** tool. Leaving a user unable to boot the OS they had before running it is the worst outcome available, and it is strictly worse than carrying one superseded-but-unrevoked CA.

> **Review date: 19 October 2026** (`Microsoft Windows Production PCA 2011` `notAfter`). Revisit this policy once Microsoft's boot-manager rollout has completed and the PCA 2011 revocation ships in the default `dbx` rather than the opt-in `DBXUpdate2024` package.
...
├── scripts/                    # Build-time helpers (payload prep, policy guard, tests)
├── sb_enema/
│   ├── secureboot-templates/   # SB-ENEMA's own keystore template (what gets enrolled)
│   └── patches/linux/          # Pinned kernel tarball hash
├── third_party/
...
1. Pulls Microsoft's `secureboot_objects` repo (certificates, scripts, pre-signed objects)
2. Generates firmware payloads (PK/KEK/db/dbx) using Microsoft's Python tooling, driven by SB-ENEMA's own keystore template at `sb_enema/secureboot-templates/SbEnemaRecovery.toml` — kept outside the submodule so a submodule bump cannot silently change which certificates get enrolled
3. Verifies the result against `scripts/check-secureboot-policy.py`, which fails the build if `db` is missing a certificate machines need to boot, or if `dbx` revokes a certificate `db` depends on
4. Packages everything into a hybrid GPT image with:
