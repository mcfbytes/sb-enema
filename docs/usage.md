...
## Prerequisites
- Host tools: `curl`, `openssl`, `mkfs.exfat` (from `exfatprogs`), `tar`, `rsync`, `sudo`, `python3-venv`, `git` with submodule support (GitHub Actions workflows install these automatically).
...
3. The init script mounts the `SB-ENEMA` exFAT partition at `/mnt/data` and runs `sb-enema`.
