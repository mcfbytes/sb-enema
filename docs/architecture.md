...
- Kernel fragment enables EFI stub boot, EFI variable access, FAT32 and exFAT, loop devices, and necessary NLS support.
...
- `scripts/prepare-secureboot-objects.sh` installs python deps for the `secureboot_objects` submodule, runs `secure_boot_default_keys.py` to emit firmware payloads, and stages both the generated payloads and source certificates/scripts/templates into `output/secureboot-staging` for inclusion in the FAT32 data partition. It also copies `PostSignedObjects/KEK/kek_update_map.json` to `output/secureboot-staging/sb-enema/kek_update_map.json` so the runtime vendor-cert filter in `stage_bios_entries()` can look up recognized OEM PK fingerprints.
