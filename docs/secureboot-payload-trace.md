...
- Microsoft PK Recovery path  
  `third_party/secureboot_objects` → `scripts/prepare-secureboot-objects.sh` (venv + `secure_boot_default_keys.py`) → `output/secureboot-staging/secureboot_artifacts/Firmware/{PK,KEK,DB,DBX}.bin` and `sb-enema/payloads/microsoft/{PK,KEK,db,dbx}.auth` → `post-image.sh` rsyncs staging onto exFAT → runtime paths `/mnt/data/secureboot_artifacts/*` and `/mnt/data/sb-enema/payloads/microsoft/*.auth` → `msft_enroll` stages certs from `/mnt/data/PreSignedObjects` into `/mnt/data/sb-enema/payloads/{PK,KEK,db}` for preview → `efi-updatevar` writes db → dbx → KEK → PK.
- Custom Owner path  
...
- Common assets  
  Templates/scripts from `secureboot_objects` land at `/mnt/data/PreSignedObjects`, `/mnt/data/Templates`, `/mnt/data/scripts` for transparency/debugging; exFAT seed adds `/PK`, `/KEK`, `/DB`, `/DBX`, `/logs` placeholders.
...
| Medium | Build tooling | sb_enema/board/sb-enema/post-image.sh | Requires host `mkfs.exfat` (exfatprogs) but Buildroot only builds target exfatprogs; build will stop if host lacks it. | Document/install host exfatprogs or add host package wiring if desired. |
...
- Build-time
  ...
  - [ ] host `mkfs.exfat` (required by post-image.sh; install `exfatprogs` on host)
...
- Runtime (per defconfig + kernel fragment)
  ...
  - [x] blkid/mount + exFAT kernel support (kernel fragment and util-linux dep)
